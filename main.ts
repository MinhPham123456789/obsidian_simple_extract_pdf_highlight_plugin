import { App, Editor, MarkdownView, TFile, Vault, Plugin, PluginSettingTab, Setting, loadPdfJs, Notice } from 'obsidian';
...
* *highlighted by ${'author'} at page ${'pageNumber'} on [[${'filepath'}#page=${'pageNumber'}]]*

`

const note = template`${'body'}
  
* *noted by ${'author'} at page ${'pageNumber'} on [[${'filepath'}#page=${'pageNumber'}]]*
...
export default class PDFAnnotationPlugin extends Plugin {
	...
	async loadSinglePDFFile(file : TFile, parentPath: string) {
		...
		filePath = parentPath + "/Annotations for " + filePath;
		await this.saveHighlightsToFile(filePath, finalMarkdown);
		await this.app.workspace.openLinkText(filePath, '', true);						
	}

	async extract_single(){
		const file = this.app.workspace.getActiveFile();
		if (file != null && file != undefined && file.extension === 'pdf') {
			...
			const warning_message = new Notice("Must open a pdf document!", 2000)
			return false
		}
	}
